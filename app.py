...
with tab5:
    ...
    if df.empty:
        ...
        # AI suggest button (full width — errors are visible)
        if gemini_available():
            ai_btn_iu = st.button("✨ AI suggest Intended Use", key=f"iu_ai_{idx}", use_container_width=False)
            if ai_btn_iu:
                ...
                if suggestion.startswith("ERROR"):
                    ...
                    st.success("✅ AI suggestion generated — see text box below.")
        else:
        ...
        if gemini_available():
            ai_btn_fr = st.button("✨ AI suggest Functional Requirements", key=f"fr_ai_{idx}")
            if ai_btn_fr:
                with st.spinner("Asking Gemini..."):
...
with tab6:
    ...
    if df.empty:
        ...
        if gemini_available() and risk_final:
            ai_btn_rj = st.button("✨ AI suggest Risk Justification", key=f"rj_ai_{tidx}")
            if ai_btn_rj:
                with st.spinner("Asking Gemini..."):
        ...
        if gemini_available():
            ai_btn_imp = st.button("✨ AI suggest Impact if Tool Fails", key=f"imp_ai_{tidx}")
            if ai_btn_imp:
                with st.spinner("Asking Gemini..."):
...
st.divider()
st.caption("v5.4 • Full AI diagnostic + error visibility • Safety filter handling • Gemini 2.5 Flash-Lite • IEC 62304 §5.3.3 + §5.1.4")
