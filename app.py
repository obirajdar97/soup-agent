...
with tab5:
    ...
    if df.empty:
            ...
            if cc == 0:
                ...
                if gemini_available():
                    if st.button("✨ AI-explain CVEs in plain English", key=f"cve_ai_{idx}"):
                        ...
                        st.session_state[f"cve_explain_{idx}"] = explanation
                
                cve_explain_existing = item.get("CVE Plain English", "")
                cve_explain_new = st.session_state.get(f"cve_explain_{idx}", "")
            ...
            if gemini_available():
                if st.button("✨ AI suggest", key=f"iu_ai_{idx}"):
                    ...
                    if not suggestion.startswith("ERROR"):
                        st.session_state[f"iu_ai_{idx}"] = f"[AI-suggested] {suggestion}"
                    else:
                        st.error(suggestion)
            else:
        ...
        iu_ai = st.session_state.get(f"iu_ai_{idx}", "")
        with col_iu_main:
        ...
        with col_fr_ai:
            if gemini_available():
                if st.button("✨ AI suggest", key=f"fr_ai_{idx}"):
                    ...
                    if not suggestion.startswith("ERROR"):
                        st.session_state[f"fr_ai_{idx}"] = f"[AI-suggested]\n{suggestion}"
                    else:
                        st.error(suggestion)
        
        fr_existing = item.get("Functional Requirements", "")
        fr_ai = st.session_state.get(f"fr_ai_{idx}", "")
        with col_fr_main:
        ...
        with col_save:
            if st.button("💾 Save", type="primary", key=f"save_{idx}"):
                ...
                for k in [f"iu_ai_{idx}", f"fr_ai_{idx}", f"cve_explain_{idx}"]:
        ...
        with col_rj_ai:
            if gemini_available() and risk_final:
                if st.button("✨ AI suggest", key=f"rj_ai_{tidx}"):
                    ...
                    if not suggestion.startswith("ERROR"):
                        st.session_state[f"rj_ai_{tidx}"] = f"[AI-suggested] {suggestion}"
                    else:
                        st.error(suggestion)
        rj_ai = st.session_state.get(f"rj_ai_{tidx}", "")
        ...
        with col_imp_ai:
            if gemini_available():
                if st.button("✨ AI suggest", key=f"imp_ai_{tidx}"):
                    ...
                    if not suggestion.startswith("ERROR"):
                        st.session_state[f"imp_ai_{tidx}"] = f"[AI-suggested] {suggestion}"
                    else:
                        st.error(suggestion)
        imp_ai = st.session_state.get(f"imp_ai_{tidx}", "")
            ...
            if st.button("💾 Save", type="primary", key=f"tsave_{tidx}"):
                ...
                with st.spinner("Saving..."):
                    update_user_fields(TOOL_SHEET, item["Name"], str(item["Version"]),
                                       item["Ecosystem"], updates)
                for k in [f"rj_ai_{tidx}", f"imp_ai_{tidx}"]:
                    if k in st.session_state: del st.session_state[k]
                st.success("✅ Saved.")
        with col_dl:
...
st.divider()
st.caption("v5.2 • Updated Gemini model (2.5 Flash-Lite) with auto-fallback • Sheet read caching • IEC 62304 §5.3.3 + §5.1.4 • FDA CSA")
