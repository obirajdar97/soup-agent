...
def log_refresh(trigger, items, new_cves, notes=""):
    ...
    try:
        ...
        if existing_data is not None:
            ...
            prev_count = int(existing_data.get("Items Refreshed", 0) or 0)
            prev_cves = int(existing_data.get("New CVEs Found", 0) or 0)
            ...
            ws.update_cell(current_week_row_idx, 1, monday)
            time.sleep(0.1)
            ws.update_cell(current_week_row_idx, 2, "weekly_digest")
            time.sleep(0.1)
            ws.update_cell(current_week_row_idx, 3, prev_count + items)
            time.sleep(0.1)
            ws.update_cell(current_week_row_idx, 4, prev_cves + new_cves)
            time.sleep(0.1)
            ws.update_cell(current_week_row_idx, 5, combined_notes)
        else:
    ...
    if df.empty:
        ...
        with st.expander("🛡️ Vulnerabilities", expanded=cc > 0):
            if cc == 0:
                ...
                cc_count = int(item.get("CVE Count", 0) or 0)
                if gemini_available():
...
st.divider()
st.caption("v6.0 • Bug fixes + Risk-based Periodic Review • FDA CSA 2025/2026 Final • Gemini 2.5 Flash-Lite • IEC 62304 §5.3.3 + §5.1.4")
