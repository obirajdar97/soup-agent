...
def call_gemini(prompt: str, max_tokens: int = 500) -> str:
    ...
    Tries each model in GEMINI_MODELS_FALLBACK until one works. This protects
    against future Google model retirements without code changes.
    """
    ...
    payload = {
        ...
        "generationConfig": {
            "temperature": 0.3,  # Low for factual, consistent regulatory text
            "maxOutputTokens": max_tokens,
        }
    }
    
    last_error = "ERROR: No models available"
    for model_name in GEMINI_MODELS_FALLBACK:
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                last_error = "ERROR: Empty response from Gemini"
            elif r.status_code == 429:
                # Rate limit — don't try other models, just report it
                return "ERROR: Rate limit exceeded. Wait a minute and try again. (Free tier: 15 req/min, 1000/day)"
            elif r.status_code == 404:
                # Model not found — try next one in the chain
                last_error = f"ERROR: Model {model_name} not available, trying fallback..."
                continue
            else:
                last_error = f"ERROR: Gemini API returned {r.status_code}. {r.text[:200]}"
                # For non-404 errors, don't keep retrying
                break
        except Exception as e:
    ...
    if df.empty:
        ...
        with st.expander("🛡️ Vulnerabilities", expanded=cc > 0):
            if cc == 0:
                ...
                # AI explain CVEs
                if gemini_available():
                    if st.button("✨ AI-explain CVEs in plain English", key=f"cve_ai_{idx}"):
                        cve_text = item.get("CVE List", "")
                        # Just explain the first CVE for now
                        ...
                        st.session_state[f"_ai_draft_cve_{idx}"] = explanation
                
                cve_explain_existing = item.get("CVE Plain English", "")
                cve_explain_new = st.session_state.get(f"_ai_draft_cve_{idx}", "")
                cve_explain_text = cve_explain_new if cve_explain_new else cve_explain_existing
                cve_explain = st.text_area(
                    "Plain-English CVE explanation (AI-generated, you review)",
                    value=cve_explain_text, height=100, key=f"cve_pe_{idx}",
                )
                if cve_explain_new and not cve_explain_new.startswith("ERROR"):
        ...
        col_iu_main, col_iu_ai = st.columns([4, 1])
        with col_iu_ai:
            if gemini_available():
                if st.button("✨ AI suggest", key=f"iu_ai_{idx}"):
                    ...
                    if not suggestion.startswith("ERROR"):
                        st.session_state[f"_ai_draft_iu_{idx}"] = f"[AI-suggested] {suggestion}"
                    else:
                        st.error(suggestion)
            else:
                st.caption("Enable AI in sidebar")
        
        iu_existing = item.get("Intended Use", "")
        iu_ai = st.session_state.get(f"_ai_draft_iu_{idx}", "")
        with col_iu_main:
            intended_use = st.text_area("Intended Use", value=iu_ai if iu_ai else iu_existing,
                                         height=100, key=f"iu_{idx}", label_visibility="collapsed")
        if iu_ai:
        ...
        col_fr_main, col_fr_ai = st.columns([4, 1])
        with col_fr_ai:
            if gemini_available():
                if st.button("✨ AI suggest", key=f"fr_ai_{idx}"):
                    with st.spinner("Asking Gemini..."):
                    ...
                    if not suggestion.startswith("ERROR"):
                        st.session_state[f"_ai_draft_fr_{idx}"] = f"[AI-suggested]\n{suggestion}"
                    else:
                        st.error(suggestion)
        
        fr_existing = item.get("Functional Requirements", "")
        fr_ai = st.session_state.get(f"_ai_draft_fr_{idx}", "")
        with col_fr_main:
            func_req = st.text_area("Functional Requirements", value=fr_ai if fr_ai else fr_existing,
                                     height=180, key=f"fr_{idx}", label_visibility="collapsed")
        if fr_ai:
            st.caption("✨ AI-suggested. Review, edit if needed, then save.")
        ...
        with col_save:
            if st.button("💾 Save", type="primary", key=f"save_{idx}"):
                ...
                # Clear AI session state for this item
                for k in [f"_ai_draft_iu_{idx}", f"_ai_draft_fr_{idx}", f"_ai_draft_cve_{idx}"]:
...
with tab6:
    ...
    if df.empty:
        ...
        col_rj_main, col_rj_ai = st.columns([4, 1])
        with col_rj_ai:
            if gemini_available() and risk_final:
                if st.button("✨ AI suggest", key=f"rj_ai_{tidx}"):
                    with st.spinner("Asking Gemini..."):
                    ...
                    if not suggestion.startswith("ERROR"):
                        st.session_state[f"_ai_draft_rj_{tidx}"] = f"[AI-suggested] {suggestion}"
                    else:
                        st.error(suggestion)
        rj_ai = st.session_state.get(f"_ai_draft_rj_{tidx}", "")
        rj_existing = risk_just if risk_just else item.get("Tool Risk Justification", "")
        with col_rj_main:
            risk_just_final = st.text_area("Risk Justification",
                value=rj_ai if rj_ai else rj_existing,
                height=100, key=f"rjf_{tidx}")
        if rj_ai: st.caption("✨ AI-suggested. Review before save.")
        
        # AI-powered Impact if Fails
        col_imp_main, col_imp_ai = st.columns([4, 1])
        with col_imp_ai:
            if gemini_available():
                if st.button("✨ AI suggest", key=f"imp_ai_{tidx}"):
                    with st.spinner("Asking Gemini..."):
                        suggestion = ai_impact_if_fails(
                            item["Name"], item.get("Description", ""), tool_func
                        )
                    if not suggestion.startswith("ERROR"):
                        st.session_state[f"_ai_draft_imp_{tidx}"] = f"[AI-suggested] {suggestion}"
                    else:
                        st.error(suggestion)
        imp_ai = st.session_state.get(f"_ai_draft_imp_{tidx}", "")
        imp_existing = item.get("Impact if Tool Fails", "")
        with col_imp_main:
            impact = st.text_area("Impact if Tool Fails",
                value=imp_ai if imp_ai else imp_existing,
                height=100, key=f"imp_{tidx}")
        if imp_ai: st.caption("✨ AI-suggested. Review before save.")
            ...
            if st.button("💾 Save", type="primary", key=f"tsave_{tidx}"):
                ...
                with st.spinner("Saving..."):
                    update_user_fields(TOOL_SHEET, item["Name"], str(item["Version"]),
                                       item["Ecosystem"], updates)
                for k in [f"_ai_draft_rj_{tidx}", f"_ai_draft_imp_{tidx}"]:
                    if k in st.session_state: del st.session_state[k]
                st.success("✅ Saved.")
        with col_dl:
...
st.divider()
st.caption("v5.3 • Fixed AI suggest session state bug • Gemini 2.5 Flash-Lite • Cached reads • IEC 62304 §5.3.3 + §5.1.4")
