...
def call_gemini(prompt: str, max_tokens: int = 500) -> str:
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            return "ERROR: Rate limit exceeded (15 req/min on free tier). Wait a minute and try again."
        else:
            return f"ERROR: Gemini API returned {r.status_code}. {r.text[:200]}"
    except Exception as e:
...
st.divider()
st.caption("v5.1 • Cached sheet reads to respect API limits • Gemini AI • IEC 62304 §5.3.3 + §5.1.4 • FDA CSA")
