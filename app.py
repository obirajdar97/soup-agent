    ...
    if df.empty:
        ...
        if gemini_available():
            if st.button("✨ AI suggest Intended Use", key=f"iu_ai_btn_{idx}"):
                with st.spinner("Asking Gemini (may take up to 30s if rate-limited)..."):
...
st.divider()
st.caption("v6.3 • Exponential backoff retry on Gemini rate limits • Batched sheet writes • Periodic Review • FDA CSA 2025/2026 Final • IEC 62304 §5.3.3 + §5.1.4")
